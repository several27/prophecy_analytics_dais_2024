...
total_clicks_impressions_spend AS (
  ...
  {#Aggregates total clicks, impressions, and spend for each account, along with other relevant details.#}
  SELECT 
    account_id,
...
formatted_spend_by_account AS (

  {#Formats spend data by account, including total clicks, impressions, spend, and other relevant information. The spend is displayed in the respective currency symbol (€ for EUR, $ for USD).#}
  SELECT 
  ...
  FROM total_clicks_impressions_spend

)

SELECT *

FROM formatted_spend_by_account
