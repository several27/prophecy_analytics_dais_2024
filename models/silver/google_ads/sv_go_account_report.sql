...
by_account_id_source_relation AS (
  ...
  {#Combines account statistics with historical account data for comprehensive performance analysis.#}
  SELECT 
...
account_performance_summary AS (
  ...
  {#Summarizes advertising account performance metrics, including total spend, clicks, and impressions.#}
  SELECT 
    account_id,
...
account_performance_summary_1 AS (
  ...
  {#Summarizes account performance metrics, including spending and engagement details.#}
  SELECT 
    ...
    CONCAT(currency_code, ' ', FORMAT_NUMBER(TOTAL_SPENT, 2)) AS formatted_spend
