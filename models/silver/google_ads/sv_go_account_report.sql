...
total_impressions_spent_clicks_by_account AS (

  {#Aggregates total impressions, spend, and clicks by account, providing summary metrics such as date, ad network type, device, currency code, and account details.#}
  SELECT 
    account_id,
...
total_impressions_spent_clicks_by_account_formatted_spend AS (

  {#Formats the total impressions, total spent, and total clicks by account, including the account details and the formatted spend amount in the respective currency.#}
  SELECT 
    ...
    CASE
      WHEN currency_code = 'USD'
        THEN CONCAT('$', TOTAL_SPENT)
      WHEN currency_code = 'EUR'
        THEN CONCAT('€', TOTAL_SPENT)
      WHEN currency_code = 'GBP'
        THEN CONCAT('£', TOTAL_SPENT)
      ELSE CONCAT(currency_code, ' ', TOTAL_SPENT)
    END AS formatted_spend
  
  FROM total_impressions_spent_clicks_by_account
...
limit_100 AS (
  ...
  FROM total_impressions_spent_clicks_by_account_formatted_spend
